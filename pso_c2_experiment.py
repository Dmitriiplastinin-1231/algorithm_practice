"""
Эксперимент PSO: влияние социального коэффициента c2.

Все параметры зафиксированы, кроме c2:
...
Диапазон c2: от 1.96 до 3.50 с шагом 0.1
  (требование: c1 + c2 > 4, т.е. c2 > 1.95)

Результаты: pso_c2_experiment.csv + pso_c2_experiment.png
"""
...
# ========================================================
# c2 > 4 - c1 = 4 - 2.05 = 1.95, поэтому начинаем с 1.96
C2_VALUES = [round(v, 2) for v in np.arange(1.96, 3.51, 0.1)]
RUNS_PER_VALUE = 50
SUCCESS_THRESHOLD = -19.0   # глобальный минимум ≈ -19.2085

CSV_FILE = "pso_c2_experiment.csv"
PNG_FILE = "pso_c2_experiment.png"
...
def run_experiment() -> list:
    ...
    for c2 in C2_VALUES:
        ...
        print(
            f"c2 = {c2:.2f}  χ = {chi:.4f}  |  среднее = {mean_best:10.6f}  |  "
...
def save_csv(results: list) -> None:
    fieldnames = ["c2", "chi", "mean_best", "best_of_best",
...
def plot_results(results: list) -> None:
    ...
    fig.suptitle(
        f"Влияние социального коэффициента c2 на PSO\n"
        f"(фиксированные: c1={C1}, num_particles={NUM_PARTICLES}, "
    ...
    ax = axes[0, 0]
    ax.plot(c2_vals, mean_bests, marker="o", linewidth=2, markersize=6,
            color="steelblue", label="Среднее лучшее")
    ax.plot(c2_vals, best_of_bests, marker="s", linewidth=2, markersize=6,
    ...
    ax.set_xlabel("Социальный коэффициент c2", fontsize=11)
    ...
    ax.bar([str(v) for v in c2_vals], success_rates, color="steelblue", alpha=0.8)
    ax.axvline(x=str(c2_vals[best_idx]), color="tomato", linestyle="--", linewidth=1.5,
               label=f"Лучший c2: {c2_vals[best_idx]}")
    ax.set_xlabel("Социальный коэффициент c2", fontsize=11)
    ...
    ax = axes[1, 0]
    ax.plot(c2_vals, conv_iters, marker="^", linewidth=2, markersize=6,
            color="seagreen")
    ax.set_xlabel("Социальный коэффициент c2", fontsize=11)
    ...
    ax = axes[1, 1]
    ax.plot(c2_vals, times_ms, marker="D", linewidth=2, markersize=6,
            color="mediumpurple")
    ax.set_xlabel("Социальный коэффициент c2", fontsize=11)
...
if __name__ == "__main__":
    print("=" * 70)
    print("  Эксперимент: влияние социального коэффициента c2 на PSO")
    ...
    print(f"    диапазон c2      = {C2_VALUES[0]} – {C2_VALUES[-1]}")
    print("=" * 70)
